...
let write_file_cost path graph =
  ...
  let _ =
    e_fold graph
      (fun count arc ->
         ...
         fprintf ff "e %d %d %d %s,%s\n" arc.src arc.tgt count flow cost;
         count + 1)
      0
  in
...
let export_cost path graph = 
  ...
  e_iter graph (fun arc ->
    let flow, cost = arc.lbl in
    fprintf ff "\t%d -> %d [ label = \"%s,%s\" ];\n" arc.src arc.tgt flow cost
  );
